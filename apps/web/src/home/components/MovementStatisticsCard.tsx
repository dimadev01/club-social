...
import { useMovementBalance } from '../useMovementBalance';
...
export function MovementStatisticsCard({ dateRange }: Props) {
  ...
  const { data: balance, isLoading: isLoadingBalance } = useMovementBalance();
    ...
    <Card size="small" title="Movimientos en el período" type="inner">
      ...
      <Card.Grid className="w-full">
        <Statistic
          ...
          title="Saldo Total"
          value={NumberFormat.formatCurrencyCents(balance?.balance ?? 0)}
        />
      </Card.Grid>
    </Card>
  );
