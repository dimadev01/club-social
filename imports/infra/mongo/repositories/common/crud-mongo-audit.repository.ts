...
export abstract class CrudMongoAuditRepository<
  TModel extends Model,
  TEntity extends Entity,
  TAuditableEntity extends AuditableEntity,
> extends CrudMongoRepository<TModel, TEntity> {
  public constructor(
    ...
    protected readonly historicalCollection: MongoCollection<TAuditableEntity>,
  ) {
  ...
  private async _save(model: TModel): Promise<void> {
    try {
      ...
      const auditableEntity: AuditableEntity = {
        ...entity,
        _id: Random.id(),
        parentId: entity._id,
      };
      ...
      // @ts-expect-error
      await this.historicalCollection.insertAsync(auditableEntity);
    } catch (error) {
