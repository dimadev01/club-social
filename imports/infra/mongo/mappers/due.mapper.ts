    ...
    return new Due(
      {
        ...
        payments: entity.payments.map<IDuePayment>((payment) => ({
          creditAmount: Money.from({ amount: payment.creditAmount }),
          directAmount: Money.from({ amount: payment.directAmount }),
          paymentDate: new DateVo(payment.paymentDate),
          paymentId: payment.paymentId,
          paymentReceiptNumber: payment.paymentReceiptNumber,
          paymentStatus: payment.paymentStatus,
          source: payment.source,
          totalAmount: Money.from({ amount: payment.totalAmount }),
        })),
        status: entity.status,
        totalPaidAmount: Money.from({ amount: entity.totalPaidAmount }),
        totalPendingAmount: Money.from({ amount: entity.totalPendingAmount }),
